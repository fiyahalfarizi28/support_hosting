...
class Historyrfm_controller extends CI_Controller {
    function __construct() {
        parent::__construct();
        $this->load->model('historyrfm_model');
	...
	public function index()
	{
        if($this->auth_model->logged_id()) {
            ...
            $this->template->load('template','history/tablerfm',$data);
        } else {
    ...
    function get_tb_detail()
    {
        ...
        $area_bandung = $this->historyrfm_model->get_crud($array_crud)->row();
        
        $array_crud = array(
            'table' => TB_PARAMETER,
            'where' => array('id' => 'RFM_AKSES_OP_APP_JABODETABEK')
        );
        $area_jabodetabek = $this->historyrfm_model->get_crud($array_crud)->row();
        
        $array_crud = array(
            'table' => TB_PARAMETER,
            'where' => array('id' => 'RFM_AKSES_IT_APP')
        );
        $area_it = $this->historyrfm_model->get_crud($array_crud)->row();
        ...
        $list = $this->historyrfm_model->get_datatables($SESSION_UPLINE);
        $data = array();
        $no = $_POST['start'];
        ...
        foreach ($list as $field) {
            ...
            $row_problem_type = $this->historyrfm_model->get_crud($array_crud)->row()->problem_type;
            ...
            $row_request_type = $this->historyrfm_model->get_crud($array_crud)->row()->request_type;
            ...
            $team_it = $this->historyrfm_model->get_crud($array_crud)->row()->value;
            ...
            if($field->project_id != NULL)
            {
                ...
                $row_project = $this->historyrfm_model->get_crud($array_crud)->row();
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->historyrfm_model->count_all(),
            "recordsFiltered" => $this->historyrfm_model->count_filtered($SESSION_UPLINE),
            "data" => $data,
        );
