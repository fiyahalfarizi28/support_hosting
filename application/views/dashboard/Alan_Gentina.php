...
<div class="card mb-3" style="margin-top: 15px">
    ...
    <div class="card-body">
        <?php
            ...
            $query_lewat_rfm = $this->db->query("SELECT COUNT(done_date) AS LEWAT_RFM FROM rfm_new_detail WHERE assign_to='1464' AND DATE(done_date) > DATE(target_date) AND request_status='DONE'")->row()->LEWAT_RFM;
            $rfm_lewat = $query_lewat_rfm;

            $query_lewat_task = $this->db->query("SELECT COUNT(done_date) AS LEWAT_TASK FROM task WHERE assign_to='1464' AND DATE(done_date) > DATE(target_date) AND status='DONE'")->row()->LEWAT_TASK;
